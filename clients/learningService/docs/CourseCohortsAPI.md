...
```swift
    open class func patchCourseCohortAsync(tenantId: UUID, cohortId: String, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a course cohort
CourseCohortsAPI.patchCourseCohortAsync(tenantId: tenantId, cohortId: cohortId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
