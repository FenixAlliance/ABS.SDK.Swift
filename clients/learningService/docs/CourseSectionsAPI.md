...
```swift
    open class func patchCourseSectionAsync(tenantId: UUID, sectionId: String, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a course section
CourseSectionsAPI.patchCourseSectionAsync(tenantId: tenantId, sectionId: sectionId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
