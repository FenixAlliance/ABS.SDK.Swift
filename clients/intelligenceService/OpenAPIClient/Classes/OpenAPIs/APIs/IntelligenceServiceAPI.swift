...
open class IntelligenceServiceAPI {
     ...
     */
    @discardableResult
    open class func invokeAgentSurfaceAsync(agentId: String, apiResponseQueue: DispatchQueue = OpenAPIClientAPI.apiResponseQueue, completion: @escaping ((_ data: Void?, _ error: Error?) -> Void)) -> RequestTask {
        return invokeAgentSurfaceAsyncWithRequestBuilder(agentId: agentId).execute(apiResponseQueue) { result in
            switch result {
            case .success:
                completion((), nil)
    ...
    /**
     Run a governed agent over the AG-UI protocol
     - POST /api/v2/IntelligenceService/Agents/{agentId}/agui
     ...
     */
    open class func invokeAgentSurfaceAsyncWithRequestBuilder(agentId: String) -> RequestBuilder<Void> {
        var localVariablePath = "/api/v2/IntelligenceService/Agents/{agentId}/agui"
