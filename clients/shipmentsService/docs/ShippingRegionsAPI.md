...
```swift
    open class func patchShippingRegionAsync(tenantId: UUID, regionId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a shipping region
ShippingRegionsAPI.patchShippingRegionAsync(tenantId: tenantId, regionId: regionId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
