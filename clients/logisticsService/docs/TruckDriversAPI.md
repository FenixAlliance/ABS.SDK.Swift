...
```swift
    open class func patchTruckDriverAsync(tenantId: UUID, driverId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a truck driver
TruckDriversAPI.patchTruckDriverAsync(tenantId: tenantId, driverId: driverId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
