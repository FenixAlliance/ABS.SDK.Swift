...
```swift
    open class func patchSupplierProfileAsync(tenantId: UUID, supplierProfileId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a supplier profile
SupplierProfilesAPI.patchSupplierProfileAsync(tenantId: tenantId, supplierProfileId: supplierProfileId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
