//
// WebsiteThemeUpdateDto.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation
#if canImport(AnyCodable)
import AnyCodable
#endif

public struct WebsiteThemeUpdateDto: Codable, JSONEncodable, Hashable {

    static let nameRule = StringRule(minLength: 0, maxLength: 255, pattern: nil)
    static let descriptionRule = StringRule(minLength: 0, maxLength: 1000, pattern: nil)
    public var name: String?
    public var description: String?
    public var authorName: String?
    public var authorUrl: String?
    public var version: String?
    public var tags: String?
    public var enable: Bool?

    public init(name: String? = nil, description: String? = nil, authorName: String? = nil, authorUrl: String? = nil, version: String? = nil, tags: String? = nil, enable: Bool? = nil) {
        self.name = name
        self.description = description
        self.authorName = authorName
        self.authorUrl = authorUrl
        self.version = version
        self.tags = tags
        self.enable = enable
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case name
        case description
        case authorName
        case authorUrl
        case version
        case tags
        case enable
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(name, forKey: .name)
        try container.encodeIfPresent(description, forKey: .description)
        try container.encodeIfPresent(authorName, forKey: .authorName)
        try container.encodeIfPresent(authorUrl, forKey: .authorUrl)
        try container.encodeIfPresent(version, forKey: .version)
        try container.encodeIfPresent(tags, forKey: .tags)
        try container.encodeIfPresent(enable, forKey: .enable)
    }
}

