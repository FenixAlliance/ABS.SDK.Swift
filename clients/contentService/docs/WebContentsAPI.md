...
```swift
    open class func patchWebContentAsync(tenantId: UUID, webContentId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a web content
WebContentsAPI.patchWebContentAsync(tenantId: tenantId, webContentId: webContentId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
