...
open class EmployeesAPI {
     ...
     */
    @discardableResult
    open class func patchEmployeeAsync(tenantId: UUID, employeeId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, apiResponseQueue: DispatchQueue = OpenAPIClientAPI.apiResponseQueue, completion: @escaping ((_ data: EmptyEnvelope?, _ error: Error?) -> Void)) -> RequestTask {
        return patchEmployeeAsyncWithRequestBuilder(tenantId: tenantId, employeeId: employeeId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation).execute(apiResponseQueue) { result in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    /**
     ...
     - returns: RequestBuilder<EmptyEnvelope> 
     */
    open class func patchEmployeeAsyncWithRequestBuilder(tenantId: UUID, employeeId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil) -> RequestBuilder<EmptyEnvelope> {
        var localVariablePath = "/api/v2/HrmsService/Employees/{employeeId}"
        let employeeIdPreEscape = "\(APIHelper.mapValueToPathItem(employeeId))"
        let employeeIdPostEscape = employeeIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        localVariablePath = localVariablePath.replacingOccurrences(of: "{employeeId}", with: employeeIdPostEscape, options: .literal, range: nil)
