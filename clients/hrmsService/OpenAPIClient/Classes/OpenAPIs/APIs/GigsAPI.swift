...
open class GigsAPI {
     ...
     */
    @discardableResult
    open class func patchGigAsync(tenantId: UUID, gigId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, apiResponseQueue: DispatchQueue = OpenAPIClientAPI.apiResponseQueue, completion: @escaping ((_ data: EmptyEnvelope?, _ error: Error?) -> Void)) -> RequestTask {
        return patchGigAsyncWithRequestBuilder(tenantId: tenantId, gigId: gigId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation).execute(apiResponseQueue) { result in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    /**
     ...
     - returns: RequestBuilder<EmptyEnvelope> 
     */
    open class func patchGigAsyncWithRequestBuilder(tenantId: UUID, gigId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil) -> RequestBuilder<EmptyEnvelope> {
        var localVariablePath = "/api/v2/HrmsService/Gigs/{gigId}"
        let gigIdPreEscape = "\(APIHelper.mapValueToPathItem(gigId))"
        let gigIdPostEscape = gigIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        localVariablePath = localVariablePath.replacingOccurrences(of: "{gigId}", with: gigIdPostEscape, options: .literal, range: nil)
