...
```swift
    open class func patchEmployeeAppraisalSessionAsync(tenantId: UUID, sessionId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch an employee appraisal session
EmployeeAppraisalSessionsAPI.patchEmployeeAppraisalSessionAsync(tenantId: tenantId, sessionId: sessionId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
