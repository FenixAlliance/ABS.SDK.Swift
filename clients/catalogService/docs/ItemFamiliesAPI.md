...
```swift
    open class func patchItemFamilyAsync(tenantId: UUID, itemFamilyId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch an item family
ItemFamiliesAPI.patchItemFamilyAsync(tenantId: tenantId, itemFamilyId: itemFamilyId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
