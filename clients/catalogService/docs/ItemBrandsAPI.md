...
# **patchItemBrandAsync**
```swift
    open class func patchItemBrandAsync(tenantId: UUID, itemBrandId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: Void?, _ error: Error?) -> Void)
```
...
// Patch an item brand
ItemBrandsAPI.patchItemBrandAsync(tenantId: tenantId, itemBrandId: itemBrandId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
