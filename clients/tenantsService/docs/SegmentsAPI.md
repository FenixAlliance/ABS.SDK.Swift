...
# **patchTenantSegment**
```swift
    open class func patchTenantSegment(tenantId: UUID, tenantSegmentId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a tenant segment
SegmentsAPI.patchTenantSegment(tenantId: tenantId, tenantSegmentId: tenantSegmentId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
