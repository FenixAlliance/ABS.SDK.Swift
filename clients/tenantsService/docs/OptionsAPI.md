...
# **patchTenantOption**
```swift
    open class func patchTenantOption(tenantId: UUID, optionId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a tenant option
OptionsAPI.patchTenantOption(tenantId: tenantId, optionId: optionId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
