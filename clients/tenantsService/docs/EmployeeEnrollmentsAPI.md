...
```swift
    open class func patchTenantEmployeeEnrollmentAsync(tenantId: UUID, tenantEmployeeEnrollmentId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a tenant employee enrollment
EmployeeEnrollmentsAPI.patchTenantEmployeeEnrollmentAsync(tenantId: tenantId, tenantEmployeeEnrollmentId: tenantEmployeeEnrollmentId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
