...
```swift
    open class func patchTenantTeamContactEnrollment(tenantId: UUID, tenantTeamContactEnrollmentId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a tenant team contact enrollment
TeamContactEnrollmentsAPI.patchTenantTeamContactEnrollment(tenantId: tenantId, tenantTeamContactEnrollmentId: tenantTeamContactEnrollmentId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
