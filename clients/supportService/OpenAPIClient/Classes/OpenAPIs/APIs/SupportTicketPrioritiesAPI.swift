...
open class SupportTicketPrioritiesAPI {
     ...
     */
    @discardableResult
    open class func patchSupportTicketPriorityAsync(tenantId: UUID, supportTicketPriorityId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, apiResponseQueue: DispatchQueue = OpenAPIClientAPI.apiResponseQueue, completion: @escaping ((_ data: EmptyEnvelope?, _ error: Error?) -> Void)) -> RequestTask {
        return patchSupportTicketPriorityAsyncWithRequestBuilder(tenantId: tenantId, supportTicketPriorityId: supportTicketPriorityId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation).execute(apiResponseQueue) { result in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    /**
     ...
     - returns: RequestBuilder<EmptyEnvelope> 
     */
    open class func patchSupportTicketPriorityAsyncWithRequestBuilder(tenantId: UUID, supportTicketPriorityId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil) -> RequestBuilder<EmptyEnvelope> {
        var localVariablePath = "/api/v2/SupportService/SupportTicketPriorities/{supportTicketPriorityId}"
        let supportTicketPriorityIdPreEscape = "\(APIHelper.mapValueToPathItem(supportTicketPriorityId))"
        let supportTicketPriorityIdPostEscape = supportTicketPriorityIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        localVariablePath = localVariablePath.replacingOccurrences(of: "{supportTicketPriorityId}", with: supportTicketPriorityIdPostEscape, options: .literal, range: nil)
