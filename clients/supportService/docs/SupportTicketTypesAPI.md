...
```swift
    open class func patchSupportTicketTypeAsync(tenantId: UUID, supportTicketTypeId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a support ticket type
SupportTicketTypesAPI.patchSupportTicketTypeAsync(tenantId: tenantId, supportTicketTypeId: supportTicketTypeId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
