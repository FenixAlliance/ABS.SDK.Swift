...
```swift
    open class func patchRefundPolicyAsync(tenantId: UUID, refundPolicyId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a refund policy
RefundPoliciesAPI.patchRefundPolicyAsync(tenantId: tenantId, refundPolicyId: refundPolicyId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
