...
```swift
    open class func patchServiceCaseAsync(tenantId: UUID, serviceCaseId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: Envelope?, _ error: Error?) -> Void)
```
...
// Patch a service case
ServiceCasesAPI.patchServiceCaseAsync(tenantId: tenantId, serviceCaseId: serviceCaseId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
