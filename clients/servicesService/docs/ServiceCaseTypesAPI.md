...
```swift
    open class func patchServiceCaseTypeAsync(tenantId: UUID, serviceCaseTypeId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: Envelope?, _ error: Error?) -> Void)
```
...
// Patch a service case type
ServiceCaseTypesAPI.patchServiceCaseTypeAsync(tenantId: tenantId, serviceCaseTypeId: serviceCaseTypeId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
