//
// AppliedItemTaxRecordUpdateDto.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation
#if canImport(AnyCodable)
import AnyCodable
#endif

public struct AppliedItemTaxRecordUpdateDto: Codable, JSONEncodable, Hashable {

    static let taxPolicyIdRule = StringRule(minLength: 0, maxLength: 36, pattern: nil)
    static let invoiceIdRule = StringRule(minLength: 0, maxLength: 36, pattern: nil)
    static let itemIdRule = StringRule(minLength: 0, maxLength: 36, pattern: nil)
    static let billingItemRecordIdRule = StringRule(minLength: 0, maxLength: 36, pattern: nil)
    public var taxPolicyId: String?
    public var invoiceId: String?
    public var itemId: String?
    public var taxAmountInUSD: Double?
    public var taxBaseAmountInUSD: Double?
    public var billingItemRecordId: String?

    public init(taxPolicyId: String? = nil, invoiceId: String? = nil, itemId: String? = nil, taxAmountInUSD: Double? = nil, taxBaseAmountInUSD: Double? = nil, billingItemRecordId: String? = nil) {
        self.taxPolicyId = taxPolicyId
        self.invoiceId = invoiceId
        self.itemId = itemId
        self.taxAmountInUSD = taxAmountInUSD
        self.taxBaseAmountInUSD = taxBaseAmountInUSD
        self.billingItemRecordId = billingItemRecordId
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case taxPolicyId
        case invoiceId
        case itemId
        case taxAmountInUSD
        case taxBaseAmountInUSD
        case billingItemRecordId
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(taxPolicyId, forKey: .taxPolicyId)
        try container.encodeIfPresent(invoiceId, forKey: .invoiceId)
        try container.encodeIfPresent(itemId, forKey: .itemId)
        try container.encodeIfPresent(taxAmountInUSD, forKey: .taxAmountInUSD)
        try container.encodeIfPresent(taxBaseAmountInUSD, forKey: .taxBaseAmountInUSD)
        try container.encodeIfPresent(billingItemRecordId, forKey: .billingItemRecordId)
    }
}

