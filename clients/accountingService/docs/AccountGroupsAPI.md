...
```swift
    open class func patchAccountGroupAsync(tenantId: UUID, accountGroupId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch an account group
AccountGroupsAPI.patchAccountGroupAsync(tenantId: tenantId, accountGroupId: accountGroupId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
