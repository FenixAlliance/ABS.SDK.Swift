...
```swift
    open class func patchFiscalYearAsync(tenantId: UUID, fiscalYearId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a fiscal year
FiscalYearsAPI.patchFiscalYearAsync(tenantId: tenantId, fiscalYearId: fiscalYearId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
