...
```swift
    open class func patchMarketingCampaignAsync(tenantId: UUID, marketingcampaignId: UUID, apiVersion: String? = nil, xApiVersion: String? = nil, operation: [Operation]? = nil, completion: @escaping (_ data: EmptyEnvelope?, _ error: Error?) -> Void)
```
...
// Patch a marketing campaign
MarketingCampaignsAPI.patchMarketingCampaignAsync(tenantId: tenantId, marketingcampaignId: marketingcampaignId, apiVersion: apiVersion, xApiVersion: xApiVersion, operation: operation) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
